...
class TestDirectorBaselines:
    ...
    @pytest.mark.asyncio
    async def test_direction_execute_turn__with_notes(
        self, active_context, baseline_checker
    ):
        """Verify director notes section renders in scene direction prompt."""
        director = active_context
        director.actions["scene_direction"].enabled = True

        # Add decaying notes
        director.notes_add(text="Maintain dramatic tension.", turns=15)
        director.notes_add(text="Focus on the approaching storm.", turns=8)
        ...
        with (
            ...
            await director.direction_execute_turn(always_on=True)
        baseline_checker(
            capture_prompt(director), AGENT, "direction_execute_turn__with_notes"
        )

    @pytest.mark.asyncio
    async def test_chat_send__with_notes(self, active_context, baseline_checker):
        """Verify director notes section renders in chat prompt."""
        director = active_context

        # Add notes
        director.notes_add(text="Keep responses concise.", turns=10)
        director.notes_add(text="Emphasize character emotions.", turns=5)
        ...
        baseline_checker(
            capture_prompt(director), AGENT, "chat_send__with_notes"
        )

    @pytest.mark.asyncio
    async def test_direction_execute_turn__all_instruction_types(
        self, active_context, baseline_checker
    ):
        """Verify all three instruction types render together in scene direction."""
        director = active_context
        director.actions["scene_direction"].enabled = True

        # 1. Scene intent instructions (via intent_state)
        director.scene.intent_state.instructions = "Always prioritize player agency over narrative pacing."

        # 2. Custom instructions (global agent config)
        director.actions["scene_direction"].config["custom_instructions"].value = (
            "Prefer dialogue over narration when advancing the scene."
        )

        # 3. Director notes (per-scene, decaying)
        director.notes_add(text="The villain is about to reveal their plan.", turns=5)
        ...
        baseline_checker(
            capture_prompt(director),
            AGENT,
            "direction_execute_turn__all_instruction_types",
        )

    @pytest.mark.asyncio
    async def test_chat_send__all_instruction_types(
        self, active_context, baseline_checker
    ):
        """Verify all three instruction types render together in chat."""
        director = active_context

        # 1. Scene intent instructions
        director.scene.intent_state.instructions = "Always prioritize player agency over narrative pacing."

        # 2. Custom instructions (global agent config)
        director.actions["chat"].config["custom_instructions"].value = (
        ...
        with (
            ...
            await director.chat_send(
                chat_id=chat_id,
                message="What should the villain do next?",
            )
        baseline_checker(
            capture_prompt(director),
            AGENT,
            "chat_send__all_instruction_types",
        )
